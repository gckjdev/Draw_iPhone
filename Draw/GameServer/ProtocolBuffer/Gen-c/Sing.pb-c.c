/* Generated by the protocol buffer compiler.  DO NOT EDIT! */
/* Generated from: Sing.proto */

/* Do not generate deprecated warnings for self */
#ifndef PROTOBUF_C__NO_DEPRECATED
#define PROTOBUF_C__NO_DEPRECATED
#endif

#include "Sing.pb-c.h"
void   game__pbsong_category__init
                     (Game__PBSongCategory         *message)
{
  static Game__PBSongCategory init_value = GAME__PBSONG_CATEGORY__INIT;
  *message = init_value;
}
size_t game__pbsong_category__get_packed_size
                     (const Game__PBSongCategory *message)
{
  assert(message->base.descriptor == &game__pbsong_category__descriptor);
  return protobuf_c_message_get_packed_size ((const ProtobufCMessage*)(message));
}
size_t game__pbsong_category__pack
                     (const Game__PBSongCategory *message,
                      uint8_t       *out)
{
  assert(message->base.descriptor == &game__pbsong_category__descriptor);
  return protobuf_c_message_pack ((const ProtobufCMessage*)message, out);
}
size_t game__pbsong_category__pack_to_buffer
                     (const Game__PBSongCategory *message,
                      ProtobufCBuffer *buffer)
{
  assert(message->base.descriptor == &game__pbsong_category__descriptor);
  return protobuf_c_message_pack_to_buffer ((const ProtobufCMessage*)message, buffer);
}
Game__PBSongCategory *
       game__pbsong_category__unpack
                     (ProtobufCAllocator  *allocator,
                      size_t               len,
                      const uint8_t       *data)
{
  return (Game__PBSongCategory *)
     protobuf_c_message_unpack (&game__pbsong_category__descriptor,
                                allocator, len, data);
}
void   game__pbsong_category__free_unpacked
                     (Game__PBSongCategory *message,
                      ProtobufCAllocator *allocator)
{
  assert(message->base.descriptor == &game__pbsong_category__descriptor);
  protobuf_c_message_free_unpacked ((ProtobufCMessage*)message, allocator);
}
void   game__pbsong_category_list__init
                     (Game__PBSongCategoryList         *message)
{
  static Game__PBSongCategoryList init_value = GAME__PBSONG_CATEGORY_LIST__INIT;
  *message = init_value;
}
size_t game__pbsong_category_list__get_packed_size
                     (const Game__PBSongCategoryList *message)
{
  assert(message->base.descriptor == &game__pbsong_category_list__descriptor);
  return protobuf_c_message_get_packed_size ((const ProtobufCMessage*)(message));
}
size_t game__pbsong_category_list__pack
                     (const Game__PBSongCategoryList *message,
                      uint8_t       *out)
{
  assert(message->base.descriptor == &game__pbsong_category_list__descriptor);
  return protobuf_c_message_pack ((const ProtobufCMessage*)message, out);
}
size_t game__pbsong_category_list__pack_to_buffer
                     (const Game__PBSongCategoryList *message,
                      ProtobufCBuffer *buffer)
{
  assert(message->base.descriptor == &game__pbsong_category_list__descriptor);
  return protobuf_c_message_pack_to_buffer ((const ProtobufCMessage*)message, buffer);
}
Game__PBSongCategoryList *
       game__pbsong_category_list__unpack
                     (ProtobufCAllocator  *allocator,
                      size_t               len,
                      const uint8_t       *data)
{
  return (Game__PBSongCategoryList *)
     protobuf_c_message_unpack (&game__pbsong_category_list__descriptor,
                                allocator, len, data);
}
void   game__pbsong_category_list__free_unpacked
                     (Game__PBSongCategoryList *message,
                      ProtobufCAllocator *allocator)
{
  assert(message->base.descriptor == &game__pbsong_category_list__descriptor);
  protobuf_c_message_free_unpacked ((ProtobufCMessage*)message, allocator);
}
void   game__pbsong__init
                     (Game__PBSong         *message)
{
  static Game__PBSong init_value = GAME__PBSONG__INIT;
  *message = init_value;
}
size_t game__pbsong__get_packed_size
                     (const Game__PBSong *message)
{
  assert(message->base.descriptor == &game__pbsong__descriptor);
  return protobuf_c_message_get_packed_size ((const ProtobufCMessage*)(message));
}
size_t game__pbsong__pack
                     (const Game__PBSong *message,
                      uint8_t       *out)
{
  assert(message->base.descriptor == &game__pbsong__descriptor);
  return protobuf_c_message_pack ((const ProtobufCMessage*)message, out);
}
size_t game__pbsong__pack_to_buffer
                     (const Game__PBSong *message,
                      ProtobufCBuffer *buffer)
{
  assert(message->base.descriptor == &game__pbsong__descriptor);
  return protobuf_c_message_pack_to_buffer ((const ProtobufCMessage*)message, buffer);
}
Game__PBSong *
       game__pbsong__unpack
                     (ProtobufCAllocator  *allocator,
                      size_t               len,
                      const uint8_t       *data)
{
  return (Game__PBSong *)
     protobuf_c_message_unpack (&game__pbsong__descriptor,
                                allocator, len, data);
}
void   game__pbsong__free_unpacked
                     (Game__PBSong *message,
                      ProtobufCAllocator *allocator)
{
  assert(message->base.descriptor == &game__pbsong__descriptor);
  protobuf_c_message_free_unpacked ((ProtobufCMessage*)message, allocator);
}
void   game__pbsong_list__init
                     (Game__PBSongList         *message)
{
  static Game__PBSongList init_value = GAME__PBSONG_LIST__INIT;
  *message = init_value;
}
size_t game__pbsong_list__get_packed_size
                     (const Game__PBSongList *message)
{
  assert(message->base.descriptor == &game__pbsong_list__descriptor);
  return protobuf_c_message_get_packed_size ((const ProtobufCMessage*)(message));
}
size_t game__pbsong_list__pack
                     (const Game__PBSongList *message,
                      uint8_t       *out)
{
  assert(message->base.descriptor == &game__pbsong_list__descriptor);
  return protobuf_c_message_pack ((const ProtobufCMessage*)message, out);
}
size_t game__pbsong_list__pack_to_buffer
                     (const Game__PBSongList *message,
                      ProtobufCBuffer *buffer)
{
  assert(message->base.descriptor == &game__pbsong_list__descriptor);
  return protobuf_c_message_pack_to_buffer ((const ProtobufCMessage*)message, buffer);
}
Game__PBSongList *
       game__pbsong_list__unpack
                     (ProtobufCAllocator  *allocator,
                      size_t               len,
                      const uint8_t       *data)
{
  return (Game__PBSongList *)
     protobuf_c_message_unpack (&game__pbsong_list__descriptor,
                                allocator, len, data);
}
void   game__pbsong_list__free_unpacked
                     (Game__PBSongList *message,
                      ProtobufCAllocator *allocator)
{
  assert(message->base.descriptor == &game__pbsong_list__descriptor);
  protobuf_c_message_free_unpacked ((ProtobufCMessage*)message, allocator);
}
void   game__pbsing_opus__init
                     (Game__PBSingOpus         *message)
{
  static Game__PBSingOpus init_value = GAME__PBSING_OPUS__INIT;
  *message = init_value;
}
size_t game__pbsing_opus__get_packed_size
                     (const Game__PBSingOpus *message)
{
  assert(message->base.descriptor == &game__pbsing_opus__descriptor);
  return protobuf_c_message_get_packed_size ((const ProtobufCMessage*)(message));
}
size_t game__pbsing_opus__pack
                     (const Game__PBSingOpus *message,
                      uint8_t       *out)
{
  assert(message->base.descriptor == &game__pbsing_opus__descriptor);
  return protobuf_c_message_pack ((const ProtobufCMessage*)message, out);
}
size_t game__pbsing_opus__pack_to_buffer
                     (const Game__PBSingOpus *message,
                      ProtobufCBuffer *buffer)
{
  assert(message->base.descriptor == &game__pbsing_opus__descriptor);
  return protobuf_c_message_pack_to_buffer ((const ProtobufCMessage*)message, buffer);
}
Game__PBSingOpus *
       game__pbsing_opus__unpack
                     (ProtobufCAllocator  *allocator,
                      size_t               len,
                      const uint8_t       *data)
{
  return (Game__PBSingOpus *)
     protobuf_c_message_unpack (&game__pbsing_opus__descriptor,
                                allocator, len, data);
}
void   game__pbsing_opus__free_unpacked
                     (Game__PBSingOpus *message,
                      ProtobufCAllocator *allocator)
{
  assert(message->base.descriptor == &game__pbsing_opus__descriptor);
  protobuf_c_message_free_unpacked ((ProtobufCMessage*)message, allocator);
}
static const ProtobufCFieldDescriptor game__pbsong_category__field_descriptors[2] =
{
  {
    "name",
    1,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_STRING,
    0,   /* quantifier_offset */
    offsetof(Game__PBSongCategory, name),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "songTags",
    3,
    PROTOBUF_C_LABEL_REPEATED,
    PROTOBUF_C_TYPE_STRING,
    offsetof(Game__PBSongCategory, n_songtags),
    offsetof(Game__PBSongCategory, songtags),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
};
static const unsigned game__pbsong_category__field_indices_by_name[] = {
  0,   /* field[0] = name */
  1,   /* field[1] = songTags */
};
static const ProtobufCIntRange game__pbsong_category__number_ranges[2 + 1] =
{
  { 1, 0 },
  { 3, 1 },
  { 0, 2 }
};
const ProtobufCMessageDescriptor game__pbsong_category__descriptor =
{
  PROTOBUF_C__MESSAGE_DESCRIPTOR_MAGIC,
  "game.PBSongCategory",
  "PBSongCategory",
  "Game__PBSongCategory",
  "game",
  sizeof(Game__PBSongCategory),
  2,
  game__pbsong_category__field_descriptors,
  game__pbsong_category__field_indices_by_name,
  2,  game__pbsong_category__number_ranges,
  (ProtobufCMessageInit) game__pbsong_category__init,
  NULL,NULL,NULL    /* reserved[123] */
};
static const ProtobufCFieldDescriptor game__pbsong_category_list__field_descriptors[1] =
{
  {
    "categorys",
    1,
    PROTOBUF_C_LABEL_REPEATED,
    PROTOBUF_C_TYPE_MESSAGE,
    offsetof(Game__PBSongCategoryList, n_categorys),
    offsetof(Game__PBSongCategoryList, categorys),
    &game__pbsong_category__descriptor,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
};
static const unsigned game__pbsong_category_list__field_indices_by_name[] = {
  0,   /* field[0] = categorys */
};
static const ProtobufCIntRange game__pbsong_category_list__number_ranges[1 + 1] =
{
  { 1, 0 },
  { 0, 1 }
};
const ProtobufCMessageDescriptor game__pbsong_category_list__descriptor =
{
  PROTOBUF_C__MESSAGE_DESCRIPTOR_MAGIC,
  "game.PBSongCategoryList",
  "PBSongCategoryList",
  "Game__PBSongCategoryList",
  "game",
  sizeof(Game__PBSongCategoryList),
  1,
  game__pbsong_category_list__field_descriptors,
  game__pbsong_category_list__field_indices_by_name,
  1,  game__pbsong_category_list__number_ranges,
  (ProtobufCMessageInit) game__pbsong_category_list__init,
  NULL,NULL,NULL    /* reserved[123] */
};
static const ProtobufCFieldDescriptor game__pbsong__field_descriptors[6] =
{
  {
    "songId",
    1,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_STRING,
    0,   /* quantifier_offset */
    offsetof(Game__PBSong, songid),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "name",
    2,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_STRING,
    0,   /* quantifier_offset */
    offsetof(Game__PBSong, name),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "author",
    3,
    PROTOBUF_C_LABEL_OPTIONAL,
    PROTOBUF_C_TYPE_STRING,
    0,   /* quantifier_offset */
    offsetof(Game__PBSong, author),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "lyric",
    4,
    PROTOBUF_C_LABEL_OPTIONAL,
    PROTOBUF_C_TYPE_STRING,
    0,   /* quantifier_offset */
    offsetof(Game__PBSong, lyric),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "lyricUrl",
    5,
    PROTOBUF_C_LABEL_OPTIONAL,
    PROTOBUF_C_TYPE_STRING,
    0,   /* quantifier_offset */
    offsetof(Game__PBSong, lyricurl),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "tag",
    20,
    PROTOBUF_C_LABEL_REPEATED,
    PROTOBUF_C_TYPE_STRING,
    offsetof(Game__PBSong, n_tag),
    offsetof(Game__PBSong, tag),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
};
static const unsigned game__pbsong__field_indices_by_name[] = {
  2,   /* field[2] = author */
  3,   /* field[3] = lyric */
  4,   /* field[4] = lyricUrl */
  1,   /* field[1] = name */
  0,   /* field[0] = songId */
  5,   /* field[5] = tag */
};
static const ProtobufCIntRange game__pbsong__number_ranges[2 + 1] =
{
  { 1, 0 },
  { 20, 5 },
  { 0, 6 }
};
const ProtobufCMessageDescriptor game__pbsong__descriptor =
{
  PROTOBUF_C__MESSAGE_DESCRIPTOR_MAGIC,
  "game.PBSong",
  "PBSong",
  "Game__PBSong",
  "game",
  sizeof(Game__PBSong),
  6,
  game__pbsong__field_descriptors,
  game__pbsong__field_indices_by_name,
  2,  game__pbsong__number_ranges,
  (ProtobufCMessageInit) game__pbsong__init,
  NULL,NULL,NULL    /* reserved[123] */
};
static const ProtobufCFieldDescriptor game__pbsong_list__field_descriptors[1] =
{
  {
    "songs",
    1,
    PROTOBUF_C_LABEL_REPEATED,
    PROTOBUF_C_TYPE_MESSAGE,
    offsetof(Game__PBSongList, n_songs),
    offsetof(Game__PBSongList, songs),
    &game__pbsong__descriptor,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
};
static const unsigned game__pbsong_list__field_indices_by_name[] = {
  0,   /* field[0] = songs */
};
static const ProtobufCIntRange game__pbsong_list__number_ranges[1 + 1] =
{
  { 1, 0 },
  { 0, 1 }
};
const ProtobufCMessageDescriptor game__pbsong_list__descriptor =
{
  PROTOBUF_C__MESSAGE_DESCRIPTOR_MAGIC,
  "game.PBSongList",
  "PBSongList",
  "Game__PBSongList",
  "game",
  sizeof(Game__PBSongList),
  1,
  game__pbsong_list__field_descriptors,
  game__pbsong_list__field_indices_by_name,
  1,  game__pbsong_list__number_ranges,
  (ProtobufCMessageInit) game__pbsong_list__init,
  NULL,NULL,NULL    /* reserved[123] */
};
static const float game__pbsing_opus__duration__default_value = 1;
static const float game__pbsing_opus__pitch__default_value = 1;
static const float game__pbsing_opus__formant__default_value = 1;
static const ProtobufCFieldDescriptor game__pbsing_opus__field_descriptors[7] =
{
  {
    "song",
    1,
    PROTOBUF_C_LABEL_OPTIONAL,
    PROTOBUF_C_TYPE_MESSAGE,
    0,   /* quantifier_offset */
    offsetof(Game__PBSingOpus, song),
    &game__pbsong__descriptor,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "voiceType",
    2,
    PROTOBUF_C_LABEL_OPTIONAL,
    PROTOBUF_C_TYPE_ENUM,
    offsetof(Game__PBSingOpus, has_voicetype),
    offsetof(Game__PBSingOpus, voicetype),
    &game__pbvoice_type__descriptor,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "duration",
    3,
    PROTOBUF_C_LABEL_OPTIONAL,
    PROTOBUF_C_TYPE_FLOAT,
    offsetof(Game__PBSingOpus, has_duration),
    offsetof(Game__PBSingOpus, duration),
    NULL,
    &game__pbsing_opus__duration__default_value,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "pitch",
    4,
    PROTOBUF_C_LABEL_OPTIONAL,
    PROTOBUF_C_TYPE_FLOAT,
    offsetof(Game__PBSingOpus, has_pitch),
    offsetof(Game__PBSingOpus, pitch),
    NULL,
    &game__pbsing_opus__pitch__default_value,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "formant",
    5,
    PROTOBUF_C_LABEL_OPTIONAL,
    PROTOBUF_C_TYPE_FLOAT,
    offsetof(Game__PBSingOpus, has_formant),
    offsetof(Game__PBSingOpus, formant),
    NULL,
    &game__pbsing_opus__formant__default_value,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "voiceDuration",
    20,
    PROTOBUF_C_LABEL_OPTIONAL,
    PROTOBUF_C_TYPE_FLOAT,
    offsetof(Game__PBSingOpus, has_voiceduration),
    offsetof(Game__PBSingOpus, voiceduration),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "localNativeDataURL",
    100,
    PROTOBUF_C_LABEL_OPTIONAL,
    PROTOBUF_C_TYPE_STRING,
    0,   /* quantifier_offset */
    offsetof(Game__PBSingOpus, localnativedataurl),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
};
static const unsigned game__pbsing_opus__field_indices_by_name[] = {
  2,   /* field[2] = duration */
  4,   /* field[4] = formant */
  6,   /* field[6] = localNativeDataURL */
  3,   /* field[3] = pitch */
  0,   /* field[0] = song */
  5,   /* field[5] = voiceDuration */
  1,   /* field[1] = voiceType */
};
static const ProtobufCIntRange game__pbsing_opus__number_ranges[3 + 1] =
{
  { 1, 0 },
  { 20, 5 },
  { 100, 6 },
  { 0, 7 }
};
const ProtobufCMessageDescriptor game__pbsing_opus__descriptor =
{
  PROTOBUF_C__MESSAGE_DESCRIPTOR_MAGIC,
  "game.PBSingOpus",
  "PBSingOpus",
  "Game__PBSingOpus",
  "game",
  sizeof(Game__PBSingOpus),
  7,
  game__pbsing_opus__field_descriptors,
  game__pbsing_opus__field_indices_by_name,
  3,  game__pbsing_opus__number_ranges,
  (ProtobufCMessageInit) game__pbsing_opus__init,
  NULL,NULL,NULL    /* reserved[123] */
};
const ProtobufCEnumValue game__pbvoice_type__enum_values_by_number[6] =
{
  { "VoiceTypeOrigin", "GAME__PBVOICE_TYPE__VoiceTypeOrigin", 0 },
  { "VoiceTypeTomCat", "GAME__PBVOICE_TYPE__VoiceTypeTomCat", 1 },
  { "VoiceTypeMale", "GAME__PBVOICE_TYPE__VoiceTypeMale", 2 },
  { "VoiceTypeFemale", "GAME__PBVOICE_TYPE__VoiceTypeFemale", 3 },
  { "VoiceTypeDuck", "GAME__PBVOICE_TYPE__VoiceTypeDuck", 4 },
  { "VoiceTypeChild", "GAME__PBVOICE_TYPE__VoiceTypeChild", 5 },
};
static const ProtobufCIntRange game__pbvoice_type__value_ranges[] = {
{0, 0},{0, 6}
};
const ProtobufCEnumValueIndex game__pbvoice_type__enum_values_by_name[6] =
{
  { "VoiceTypeChild", 5 },
  { "VoiceTypeDuck", 4 },
  { "VoiceTypeFemale", 3 },
  { "VoiceTypeMale", 2 },
  { "VoiceTypeOrigin", 0 },
  { "VoiceTypeTomCat", 1 },
};
const ProtobufCEnumDescriptor game__pbvoice_type__descriptor =
{
  PROTOBUF_C__ENUM_DESCRIPTOR_MAGIC,
  "game.PBVoiceType",
  "PBVoiceType",
  "Game__PBVoiceType",
  "game",
  6,
  game__pbvoice_type__enum_values_by_number,
  6,
  game__pbvoice_type__enum_values_by_name,
  1,
  game__pbvoice_type__value_ranges,
  NULL,NULL,NULL,NULL   /* reserved[1234] */
};
