...
- (void)shareViaSNS:(SnsType)type imagePath:(NSString*)imagePath
{
    ...
    if ([[self getOpusComment] length] > 0){
        text = [NSString stringWithFormat:NSLS(@"kShareMyOpusWithDescriptionText"), [self getOpusComment], snsOfficialNick, self.word.text, [ConfigManager getSNSShareSubject], [UIUtils getAppLink:[ConfigManager appId]]];
    }
    else{
