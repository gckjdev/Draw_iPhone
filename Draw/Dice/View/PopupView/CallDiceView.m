...
#define DICE_VIEW_WIDTH 28  
#define DICE_VIEW_HEIGHT 29
...
#define COUNT_LABEL_WIDTH_1 15
#define COUNT_LABEL_WIDTH_2 24
...
- (id)initWithDice:(int)dice count:(int)count
{
    ...
    if (self) {
        ...
        self.countLabel = [[[FontLabel alloc] initWithFrame:CGRectMake(0, 0, countLableWidth, CALL_DICE_VIEW_HEIGHT) fontName:[[DiceFontManager defaultManager] fontName] pointSize:25] autorelease];
        _countLabel.backgroundColor = [UIColor clearColor];
        _countLabel.text = [NSString stringWithFormat:NSLS(@"%d"), count]; 
        _countLabel.textAlignment = UITextAlignmentLeft;
        
        FontLabel *symbolLabel = [[[FontLabel alloc] initWithFrame:CGRectMake(countLableWidth, 0, SYMBOL_LABEL_WIDTH, CALL_DICE_VIEW_HEIGHT) fontName:[[DiceFontManager defaultManager] fontName] pointSize:18] autorelease];
